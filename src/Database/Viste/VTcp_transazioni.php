...
class VTcp_transazioni
{
    ...
    public function recuperaDatiPerQuadratura(array $request): string {
        ...
        $stmt = "   SELECT 'F' tipo, case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112) + '0' + right(ts.code,2) + '0' + right(ts.code,2) id, '' codice, count(*) s1, sum(t.total_amount) s2
                    ...
                    group by convert(DATE, t.trans_date), case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112) + '0' + right(ts.code,2) + '0' + right(ts.code,2)
                    UNION 
                    SELECT 'H' tipo, case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112) id, replace(CONVERT(VARCHAR(8),max(t.trans_date),108),':','')  codice, 0 s1, 0 s2
                    FROM TCPOS4.dbo.transactions t 
                        join TCPOS4.dbo.tills ts on t.till_id = ts.id 
                        join TCPOS4.dbo.shops sh on t.shop_id = sh.id 
                    where convert(DATE, t.trans_date) = '$data' $tillSearch
                    group by case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112)
                    union
                    SELECT 'T' tipo, case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112) + '0' + right(ts.code,2) + '0' + right(ts.code,2) id, 
                        case 
                    ...
                    group by case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112) + '0' + right(ts.code,2) + '0' + right(ts.code,2), 
                             case 
                                when pay.payment_id = 1 then '01'
                                when pay.payment_id = 5 then '04'
                                when pay.payment_id = 10 then '08'
                ...
                union
                select 'T' tipo, case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112) + '0' + right(ts.code,2) + '0' + right(ts.code,2) id, cc.card_number_ident codice,sum(tp.amount ) s1, 0 s2
                from TCPOS4.dbo.transactions t
                    join TCPOS4.dbo.tills ts on t.till_id = ts.id 
                    join TCPOS4.dbo.shops sh on t.shop_id = sh.id
                ...
                where convert(DATE, t.trans_date) = '$data' $tillSearch
                group by case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112) + '0' + right(ts.code,2) + '0' + right(ts.code,2), cc.card_number_ident
                union
                SELECT 'T' tipo, case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112) + '0' + right(ts.code,2) + '0' + right(ts.code,2) id, 4 codice, sum(pay.amount) s1, 0 s2
                FROM TCPOS4.dbo.transactions t 
                    join TCPOS4.dbo.tills ts on t.till_id = ts.id 
                    join TCPOS4.dbo.shops sh on t.shop_id = sh.id
                    join TCPOS4.dbo.trans_payments pay on t.id = pay.transaction_id 
                ...
                union
                select 'V' tipo, 
                    case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), 
                ...
                where convert(DATE, t.trans_date) = '$data' $tillSearch
                group by case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112) + '0' + right(ts.code,2) + '0' + right(ts.code,2), isnull(case when tv.vat_id = 1 then 2 when  tv.vat_id = 4 then 1 else tv.vat_id end, 2)
                UNION
                SELECT 
                    'C' tipo,
                    case when left(s.code, 4) = '0600' then '0502' when left(s.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), c.opening_date, 112) + '0' + right(ts.code,2)  id, 
                    0 codice, 
                ...
                group BY case when left(s.code, 4) = '0600' then '0502' when left(s.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), c.opening_date, 112) + '0' + right(ts.code,2) 
                union 
                SELECT 'I' tipo, case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112) + '0' + right(ts.code,2) id, ts.code codice, count(*) s1, sum(t.total_amount) s2
                FROM TCPOS4.dbo.transactions t 
                    join TCPOS4.dbo.tills ts on t.till_id = ts.id 
                    join TCPOS4.dbo.shops sh on t.shop_id = sh.id
                where convert(DATE, t.trans_date ) = '$data' $tillSearch
                group by case when left(sh.code, 4) = '0600' then '0502' when left(sh.code, 4) = '0700' then '0503' else '0500' end + convert(varchar(8), t.trans_date, 112) + '0' + right(ts.code,2), ts.code
                UNION
                SELECT 
                    'R' tipo, 
                    case 
    ...
    public function creazioneDatacollectTcPos(array $request): string {
        ...
        $stmt = "SELECT 
                ...
                FROM TCPOS4.dbo.transactions t 
                    ...
                    join TCPOS4.dbo.trans_articles ta on t.id = ta.transaction_id 
                    join TCPOS4.dbo.articles a on a.id = ta.article_id 
                    join TCPOS4.dbo.vats v on v.id = ta.vat_id 
